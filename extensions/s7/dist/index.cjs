...
// extensions/s7/src/builder.ts
var import_node_fs = __toESM(require("node:fs"), 1);
var import_node_path = __toESM(require("node:path"), 1);
var import_yaml = __toESM(require_dist(), 1);

// extensions/s7/src/database.ts
function normalizeSlug(slug) {
  return slug.replace(/\\/g, "/").replace(/^\/+|\/+$/g, "").toLowerCase();
}
var S7Database = class {
...
var Database = new S7Database();

// extensions/s7/src/builder.ts
function buildDatabase(files) {
  Database.clear();
  console.log("\u{1F4DA} Construyendo S7Database");
  for (const relativePath of files) {
    const absolutePath = import_node_path.default.join(process.cwd(), "content", relativePath);
    const text = import_node_fs.default.readFileSync(absolutePath, "utf8");
    let frontmatter = {};
  ...
  console.log(`\u{1F4DA} S7Database: ${Database.size()} p\xE1ginas indexadas`);
}
...
// extensions/s7/src/transformers/htmlLinks.ts
function htmlLinksTransformer() {
  return (tree) => {
...
var manifest = {
  ...
  description: "Plugin de integraci\xF3n para el sistema S7.",
  version: "0.1.0",
  ...
  components: {
    S7Hero: {
      name: "S7Hero",
      displayName: "S7 Hero",
      description: "Cabecera de las p\xE1ginas S7",
      version: "0.1.0"
    }
  }
};
